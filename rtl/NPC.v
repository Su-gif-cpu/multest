...
module NPC(
    ...
    always @(*) begin
        case(NPCOp)
            ...
            // JALR 跳转寄存器： RISC-V 规范要求 JALR 必须强行清零目标地址的最低位
            `NPC_rs       : NPC = {rs[31:1], 1'b0}; 
            
            // JAL 跳转并链接：当前PC没变，直接 PC + Offset
            `NPC_Offset20 : NPC = PC + Offset21; 
            
            default       : NPC = PC + 4;
        endcase
        
        // 用于 JAL / JALR 存回目标寄存器（ra/x1）的返回地址
        PCA4 = PC + 4; 
    end
endmodule